...
class Trie {
	...
	public List<String> getAllWordsStartingWith(String prefix){
		...
		HashMap<Character, TrieNode> currentChildren = main ;
		for(int i = 0 ; i < prefix.length(); i++){
			currentChar = prefix.charAt(i) ;
			sb.append(currentChar);
			currentNode = currentChildren.getOrDefault(currentChar, null);
			if(currentNode == null){
				break ;
			}
		...
		if(currentNode != null){
			getAllWordsFrom(currentNode, prefix, result) ;			
		}
		
		return result ;
	}
	
	public void getAllWordsFrom(TrieNode currentNode, String prefix, List<String> result){
		
		HashMap<Character, TrieNode> currentChildren = currentNode.children ;
		TrieNode currentChild = null ;
		for(Map.Entry<Character, TrieNode> entry : currentChildren.entrySet()){
			currentChild = entry.getValue() ;
			if(currentChild.isWord){
				result.add(prefix + currentChild.value) ;
			}
			getAllWordsFrom(currentChild, prefix + currentChild.value, result);
		}
	}
...
//			}
//			
//			// this condition to include words shorter than the prefix ... should be commented in case we don't want this behavior
